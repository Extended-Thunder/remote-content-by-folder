...
async function checkMessage(message) {
    ...
    if (currentPolicy != "None") {
        await debug(`Content policy for message "${message.id}" is set to "${currentPolicy}", not modifying`);
        return;
    }
    ...
    if (requestedPolicy && currentPolicy != requestedPolicy) {
        await debug(`Switching content policy for message "${message.id}" from "${currentPolicy}" to "${requestedPolicy}"`);
...
async function checkRegexp(msgHdr, prefName) {
    ...
    if (regexp != "") {
        try {
            ...
            await debug(`Testing ${prefName} regexp "${regexp}" against folder name "${msgHdr.folder.name}"`);
            if (regexpObj.test(msgHdr.folder.name)) {
                await debug(`${prefName} regexp "${regexp}" matched folder name "${msgHdr.folder.name}"`);
                return true;
            }
            await debug(`${prefName} regexp "${regexp}" did not match folder name "${msgHdr.folder.name}"`);
            return false;
        } catch (ex) {
